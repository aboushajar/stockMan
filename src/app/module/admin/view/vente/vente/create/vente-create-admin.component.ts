...
export class VenteCreateAdminComponent implements OnInit {
    ...
    findStockBySousMarque() {
        this.stockService.findBySousMarque(this.venteItemsElement.sousMarque).subscribe(res => {
            this.stocks = res
            this.stocks.forEach(e => e.description = e.depot.libelle + " (" + e.quantiteEligibleVente + ":" + e.sousMarque.prixVente + " DH)")
        })
    }
    ...
    public saveWithShowOption(showList: boolean) {
        ...
        if (!this.item.etatVente && this.item.montantPaye == this.item.montantTotalApresRemise) {
            this.item.etatVente = this.etatVentes.find(e => e.code === "Confirme");
        } else if (!this.item.etatVente && this.item.montantPaye == this.item.montantTotalApresRemise) {
            this.item.etatVente = this.etatVentes.find(e => e.code === "Pending");
        }
    ...
    private caluclateTotal() {
        ...
        if (this.item.tauxTva) {
            this.item.montantTotalHt += this.venteItemsElement.prix * this.venteItemsElement.quantite
            this.item.montantTotalTva += (this.item.montantTotalHt * this.item.tauxTva.valeur) / 100
            this.item.montantTotalTtc += this.item.montantTotalHt + this.item.montantTotalTva
    ...
    calculatePrix() {
        if (!this.venteItemsElement.quantite) {
            this.venteItemsElement.quantite = 1
        }
        if (this.venteItemsElement.stock && this.venteItemsElement.stock.sousMarque) {
            this.venteItemsElement.prix = this.venteItemsElement.stock.sousMarque.prixVente
        }
    ...
    updateMontantApresRemise() {
        ...
        this.item.montantTotalApresRemise = this.item.montantTotalTtc - this.item.remise
    }

    updateMontantPaye() {
        this.item.montantPaye = this.item.montantTotalApresRemise
        this.item.reste = this.item.montantTotalApresRemise - this.item.montantPaye
    }

    public deleteventeItems(p: VenteItemDto) {
        this.item.venteItems.forEach((element, index) => {
            if (element === p) {
                this.item.venteItems.splice(index, 1);
            }
        });
    }
    ...
    public deletereglementVentes(p: ReglementVenteDto) {
        this.item.reglementVentes.forEach((element, index) => {
            if (element === p) {
                this.item.reglementVentes.splice(index, 1);
            }
        });
    }
    ...
    public validateVenteItemsQuantite() {
        if (this.venteItemsElement.quantite == null || this.venteItemsElement.stock.quantiteEligibleVente < this.venteItemsElement.quantite) {
            this.errorMessages.push('Quantite de la venteItem est  invalide : Quantite doit etre inferieure a la quantite Stock');
    ...
    public validateVenteItemsVente() {
    ...
    public validateReglementVentesDateReglement() {
    ...
    public validateReglementVentesModeReglement() {
    ...
    public validateReglementVentesVente() {
    ...
    public async openCreateEtatVente(etatVente: string) {
        const isPermistted = await this.roleService.isPermitted('EtatVente', 'add');
        if (isPermistted) {
    ...
    findSousMarqueByMarque() {
        this.sousMarqueService.findByMarque(this.venteItemsElement.marque).subscribe(res => this.sousMarques = res);
    }
